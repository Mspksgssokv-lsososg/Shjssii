...
const oauth2ClientForGGDrive = (googleClientId && googleClientSecret && googleRefreshToken)
	? new google.auth.OAuth2(googleClientId, googleClientSecret, "https://developers.google.com/oauthplayground")
	: null;
if (oauth2ClientForGGDrive) oauth2ClientForGGDrive.setCredentials({ refresh_token: googleRefreshToken });
const driveApi = oauth2ClientForGGDrive ? google.drive({
	version: 'v3',
...
const drive = {
	...
	async uploadFile(fileName, mimeType, file) {
		if (!driveApi) throw new Error("Google Drive is not configured");
