/**
 * Telegram transport for the original GoatBot command/event system.
 *
 * The original folder/file layout is intentionally kept intact. This file
 * replaces the Messenger/FCA transport with Telegram Bot API long polling
 * while preserving the api.* surface used by the existing commands.
 */
...
function readTokenFile() {
  ...
  // Supported account.txt formats:
  //   123456:AA...
  //   BOT_TOKEN=123456:AA...
  //   {"token":"123456:AA...","adminIds":["123"]}
  try {
    if (raw.startsWith("{")) {
...
function makeAttachment(fileId, type, name, botApi) {
  ...
  // Keep the Messenger-compatible `url` property. Telegram file URLs are
  // resolved lazily so receiving a message does not block the update loop.
...
class TelegramApi {
  ...
  async sendMessage(form, threadID, callback, replyToMessageID) {
    ...
    if (form && typeof form === "object" && form.mentions) {
      ...
      // Convert known mention tags to HTML mentions when possible.
      const entities = [];
      let text = body;
    ...
    if (!attachments.length) {
      ...
      // Telegram messages are single-media. Send each attachment separately;
      // the first one receives the body as caption.
  ...
  async getUserInfo(userID) {
    ...
    // getChat is valid for private users and members whose IDs are known to the bot.
    try {
  ...
  async changeNickname(nickname, threadID, userID) {
    // Telegram has no per-member nickname API. For admins, set the custom title.
...
async function fetchGban() {
  ...
  for (const url of urls) {
    try { return (await axios.get(url, { timeout: 10000 })).data || {}; } catch (_) { /* try next */ }
  }
...
async function startBot() {
  ...
  await api.getMe();
  global.GoatBot.fcaApi = api; // compatibility name used by the original core
  ...
  // Existing auto-load script system is preserved exactly at the loader level;
  // this watcher only supplies the Telegram transport to the same loader.
  ...
  // Telegram equivalent of restartTelegramPolling: safely restart long polling.
...
log.master("SUCCESS", "Telegram bot is running");
  ...
  console.log("Project GoatBot v2 Telegram transport - original command/event structure preserved.");
}
